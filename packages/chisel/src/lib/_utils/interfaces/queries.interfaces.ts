import { AbstractFilterQuery, FilterQuery } from "../types/queries.type";
...
export interface QueryMethods<T> {
  ...
  where: (
    params: FilterQuery<T> | AbstractFilterQuery,
  ) => void | Partial<QueryMethods<T>>;
  ...
  set: (
    params: { [P in keyof T]?: T[P] } | Record<string, string>,
  ) => void | Partial<QueryMethods<T>>;
}
