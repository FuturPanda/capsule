...
//  */
//
// export class ChiselModelBackup<T> {
...
//   /**
//    *  Insert function.
//    *  example : model<T>.insert({key: value})
//    */
//   insert(
...
//   ) {
//     let query: string = `INSERT INTO ${this.modelName}`;
//     const keys = Object.keys(params);
//     const placeholders = keys.map(() => "?").join(", ");
//     query += ` (${keys.join(", ")}) VALUES (${placeholders})`;
//
...
//
//       // If specific update columns are provided, only update those
//       const columnsToUpdate = updateColumns || keys;
//       const updates = columnsToUpdate
